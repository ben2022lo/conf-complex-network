...
def draw_complex(c: SimplicialComplex, em: Embedding,
    ...
    # set up the axes
    ax.set_xlim([-1.5, 1.5])      # axes bounded around 1
    ax.set_ylim([-1.5, 1.5])
    ...
    for s in c.simplicesOfOrder(0):
        ...
        cc = ['b','g','r','c','m','y','k','w','b','g','r','c']
        circ = plt.Circle([x, y],
                          radius=node_size,
                          edgecolor='black', facecolor=cc[i],
                          zorder=3)
        ax.add_patch(circ)
        i += 1
    # draw the edges
    ...
    for s in c.simplicesOfOrder(2):  
        if abs(c.getAttributes(s)['weight'])>m :
            m = c.getAttributes(s)['weight']
    ...
    for s in c.simplicesOfOrder(2):
        ...
        (x1, y1) = em[fs[1]]
        (x2, y2) = em[fs[2]]
        if c.getAttributes(s)['weight']/m < 0.3:
            alpha = 0
        else:
            alpha=abs(c.getAttributes(s)['weight'])/m
